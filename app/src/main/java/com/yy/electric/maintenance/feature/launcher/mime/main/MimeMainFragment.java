...
import com.yy.electric.maintenance.feature.launcher.mime.MimeInfoMgr;
...
import com.yy.electric.maintenance.feature.login.LoginActivity;
import com.yy.electric.maintenance.util.LogUtil;
import com.yy.electric.maintenance.widget.CircularProgressView;
...
public class MimeMainFragment extends BaseFragment implements MimeMainContract.View, View.OnClickListener {
  ...
  private void init() {
    ...
    mOrderRequest.url = "Baoding_Overview_DataSupport/Services/GetUserRecRepairOrderQuantity?";
    ...
    mAlertQuantityRequest.url = "Baoding_Overview_DataSupport/Services/GetUserFinishRepairOrderQuantity?";
  ...
  public void updateMimeOrderData(MimeOrderData data) {
    try {
      for (MimeOrderData.Row row : data.rows) {
        if (row.result >= 0) {
          LogUtil.e(TAG, "updateMimeOrderData() row.result=" + row.result);
